...
function update_total() {
  ...
  //var forfaitfixe = $('#forfaitfixe').val();
  //total = +subtotal + +forfaitfixe;
...
function update_price() {
  ...
  var commission = row.find('.cost').val().replace("","") * 0.025; 
  commission = parseFloat(commission).toFixed(2);
  isNaN(commission) ? row.find('.commission').html("N/A") : row.find('.commission').html(""+commission);
  var price = row.find('.commission').val().replace("","") * row.find('.qty').val();
